// SECTION: Imports — live countdown timer and sale CTA
...
const SaleBanner = () => {
  // SECTION: Countdown target — sale end date and ticking time state
  const end = new Date('2025-06-01T00:00:00');
  const [time, setTime] = useState({ d: 0, h: 0, m: 0, s: 0 });

  // SECTION: Interval — recalculates days/hours/mins/secs every second
  useEffect(() => {
    const tick = () => {
  ...
  // SECTION: Display units — labels for the four countdown boxes
  const units = [
    ...
    <section className="max-w-7xl mx-auto px-4 sm:px-6 py-16">
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative overflow-hidden rounded-3xl p-10 md:p-16 animate-gradient-shift bg-gradient-to-r from-luxury-purple/40 via-luxury-bg to-luxury-gold/20 border border-white/10">
        ...
        <div className="relative z-10 grid md:grid-cols-2 gap-10 items-center">
          ...
          {/* SECTION: Countdown grid — four glass cards for d/h/m/s */}
